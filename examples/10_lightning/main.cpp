...
int main() {
    ...
    auto projection = cam.get_projection_mat();
    ...
    constexpr auto object_color = glm::vec3{1.f, 0.5f, 0.31f};
    constexpr auto light_color = glm::vec3{1.f, 1.f, 1.f};

    obj_shader.use();
    SGL_VERIFY(obj_shader.set_uniform_mat4(u_model, glm::value_ptr(model)));
    SGL_VERIFY(obj_shader.set_uniform_mat4(u_projection, glm::value_ptr(projection)));
    SGL_VERIFY(obj_shader.set_uniform_vec3(u_object_color, glm::value_ptr(object_color)));
...
void render_object(const sgl::shader &shader, const sgl::vertex_array &vao, const sgl::camera &cam) {
    ...
    vao.bind();
    glDrawElements(GL_TRIANGLES, g_indices.size(), GL_UNSIGNED_INT, nullptr);
}
